...
const RegisterPage = () => {
  ...
  const handleRequestOTP =  async (e) => {
    ...
    setLoading(true);
    try{
      const response = await  api.post('auth/register/request', {email: formData.email});
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try{
      ...
      setFormData({...formData, registration_code: response.data.data.registration_code});
      setStep(3);
    }catch(err){
      setError(err?.response?.data?.message || "Something went wrong, please try again");
      console.log(err?.response?.data?.error_code);
  ...
  return(
    <div className="flex items-center justify-center h-screen w-screen bg-sky-100">
      <div className="flex flex-col w-1/2 bg-white shadow-lg rounded-2xl p-10">
        <h1 className="flex  items-center justify-center font-bold">Register Account</h1>
        ...
        {step === 2 && (
          ...
          >
            <div className="font-bold text-lg">Email</div>
            <div className="rounded-sm p-2 my-2 bg-gray-50">{formData.email}</div>
        ...
        {step ===  3 &&(
          ...
          >
            ...
            <div className="text-sm text-sky-400">Note: After register success, you can login by username or email</div>
            <input
              type="text"
              name="username"
