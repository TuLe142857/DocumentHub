...
def test_update_collection_success(auth_client, collection):
    assert_response_ok(
        auth_client.patch(f"/collections/{collection.id}", json={"new_name": "new name"})
    )
...
def test_add_document_to_collection_success(auth_client, collection, user, db_session, category):
    ...
    assert_response_ok(auth_client.put(f"/collections/{collection.id}/items/{new_document.id}"))

def test_delete_document_from_collection_success(auth_client, collection, document):
    assert_response_ok(auth_client.delete(f"/collections/{collection.id}/items/{document.id}"))
